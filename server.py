...
# Directory where uploaded video files are stored on disk
UPLOAD_DIR = ROOT_DIR / "uploaded_videos"
...
class VideoUpload(BaseModel):
    ...
    file_path: str  # path to the actual video file saved on disk
    analyzed: bool = False
...
async def upload_video(
    ...
    """Upload a tennis video for analysis - saves the FULL video file to disk for real analysis"""
    user = await get_current_user(authorization)
...
async def analyze_video(
    ...
    """Analyze tennis video using real pose-detection (MediaPipe) - no fabricated data"""
    user = await get_current_user(authorization)
    ...
    try:
        ...
        try:
            analysis_data = pose_analyzer.analyze_swing_for_app(file_path, shot_type, dominant="R")
        except ValueError as analysis_error:
            # Real, honest error - e.g. no person detected clearly in the video
            raise HTTPException(status_code=422, detail=str(analysis_error))
